class Solution {
private:
    void dfs(vector<vector<int>>& h, set<pair<int,int>> &uset, int i, int j, int prevH){
        int rows = h.size(), cols = h[0].size();
        if(i >= rows || j >= cols || i < 0 || j < 0 || uset.find({i, j}) != uset.end() || h[i][j] < prevH){
            return;
        }
